...
def grades_window(grades_dict):
    ...
    return Sg.Window(f"{grades_dict['aluno']} - Notas IFMG - AVA Betim", layout, resizable=True)
...
while True:
    ...
    if event == Sg.WINDOW_CLOSED:
        ...
        if ra == '' or senha == '':
            Sg.popup_error("Preencha os campos RA e senha!",
                           title="Erro de login", background_color="gray", text_color="white", font="Helvitica 12 bold")
            continue

        if not ra.isnumeric() or not senha.isnumeric():
            Sg.popup_error("Os campos são numéricos!",
                           title="Erro de login", background_color="gray", text_color="white", font="Helvitica 12 bold")
            continue
        ...
        if login is None:
            Sg.popup_error("Não foi possível obter suas informações. Confira o RA e senha e tente novamente.",
                           title="Erro de login", background_color="gray", text_color="white", font="Helvitica 12 bold")
            lw.un_hide()
        else:
